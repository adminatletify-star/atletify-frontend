...
export default function RegistroManual() {
  ...
  const cargarDatos = async () => {
    ...
    try {
      ...
      // Cargar planes
      const resPlanes = await fetch(`https://localhost:7149/api/finanzas/planes/${boxActual.idBox}`, { headers });
      if (resPlanes.ok) {
      ...
      const resDescuentos = await fetch(`https://localhost:7149/api/finanzas/descuentos/${boxActual.idBox}`, { headers });
      if (resDescuentos.ok) {
      ...
      const resConfig = await fetch(`https://localhost:7149/api/configuracionbox/${boxActual.idBox}`, { headers });
      if (resConfig.ok) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await fetch('https://localhost:7149/api/usuarios/registro-manual', {
        method: 'POST',
