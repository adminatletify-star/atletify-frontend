...
export default function RegistroManual() {
  ...
  const cargarDatos = async () => {
    ...
    try {
      ...
      // Cargar planes
      const resPlanes = await fetch(`import.meta.env.VITE_API_URL:7149/api/finanzas/planes/${boxActual.idBox}`, { headers });
      if (resPlanes.ok) {
      ...
      const resDescuentos = await fetch(`import.meta.env.VITE_API_URL:7149/api/finanzas/descuentos/${boxActual.idBox}`, { headers });
      if (resDescuentos.ok) {
      ...
      const resConfig = await fetch(`import.meta.env.VITE_API_URL:7149/api/configuracionbox/${boxActual.idBox}`, { headers });
      if (resConfig.ok) {
